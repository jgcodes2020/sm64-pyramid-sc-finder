...
namespace sm64 {
  ...
  struct vec_t {
    ...
    static_assert(
      ...
      "T cannot be const or volatile"
    );
    static_assert(N >= 1, "Vector should contain at least 1 dimension");

  public:
    T& operator[](size_t i) { return _m_data[i]; }

    const T& operator[](size_t i) const { return _m_data[i]; }

    // Deleted versions (defaults)

    T& x() requires(N >= 1) { return _m_data[0]; }
    T& y() requires(N >= 2) { return _m_data[1]; }
    T& z() requires(N >= 3) { return _m_data[2]; }

    const T& x() const requires(N >= 1) { return _m_data[0]; }
    const T& y() const requires(N >= 2) { return _m_data[1]; }
    const T& z() const requires(N >= 3) { return _m_data[2]; }
    ...
    template <class U>
    requires(std::is_convertible_v<T, U>) explicit operator vec_t<U, N>(
    ) const {
      return SM64_VMATH_FOLD_RETURN(
        N, (vec_t<U, N> {static_cast<U>((*this)[Is])...})
      );
    }
  ...
  inline vec_t<T, 3> vec_cross(const vec_t<T, 3>& a, const vec_t<T, 3>& b) {
    return vec_t<T, 3> {
      a[1] * b[2] - a[2] * b[1], a[2] * b[0] - b[0] * a[2],
      a[0] * b[1] - b[0] * a[1]};
  }
  ...
  inline vec_t<T, N> vec_normalize(const vec_t<T, N>& a) {
    static_assert(
      ...
      "Normalization only makes sense for floating-point types"
    );
  ...
  struct mat4_t {
  public:
    vec_t<T, 4>& operator[](size_t n) { return _m_data[n]; }

    const vec_t<T, 4>& operator[](size_t n) const { return _m_data[n]; }
